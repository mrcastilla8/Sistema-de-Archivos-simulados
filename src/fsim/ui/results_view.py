...
# ... (Constantes METRIC_NAMES_ES, METRIC_ORDER, KEYS_TO_IGNORE, MANIFEST_COLUMNS sin cambios) ...
METRIC_NAMES_ES = {
...
class ResultsView(ctk.CTkFrame):
    ...
    def __init__(self, master, palette: Dict[str, str], **kwargs):
        ...
        # --- Creación de Widgets (Simplificada para brevedad, igual que antes) ---
        ...
        # --- MODIFICACIÓN: Ajustar ancho para texto más largo ---
        button_style_export = {"fg_color": self.palette["button"], "hover_color": self.palette["button_hover"], "text_color": self.palette["text_on_button"], "width": 220} # Ajustar ancho
        # --- FIN MODIFICACIÓN ---

        # --- MODIFICACIÓN: Cambiar texto del botón JSON ---
        ...
        # --- MODIFICACIÓN: Cambiar texto del botón CSV ---
        ...
        # Configuración de Estilo y Treeview (igual que antes)
    ...
    # --- MODIFICADO: _clear_results Correcto ---
    def _clear_results(self):
        """Limpia métricas, tabla y deshabilita botones."""
        # Limpiar widgets del frame de métricas
        # Necesitamos iterar sobre una COPIA de la lista de hijos
        # porque destruir un widget modifica la lista original
        for widget in list(self.metrics_scroll_frame.winfo_children()):
            widget.destroy()
        self.metrics_current_row = 0

        # Limpiar tabla Treeview
        # Usar get_children() que devuelve una tupla (inmutable)
    ...
    def _populate_manifest_table(self, manifest_data: List[Dict[str, Any]]):
        ...
        for item in self.tree.get_children():
            self.tree.delete(item)
        self._current_manifest_data = manifest_data # Guardar para CSV
        # Insertar filas
        ...
        # Habilitar/deshabilitar botón CSV
    ...
    def _export_manifest_csv(self):
        ...
        try:
            ...
            with open(filepath, "w", newline="", encoding="utf-8") as f:
                ...
                writer.writerow(MANIFEST_COLUMNS) # Encabezados traducidos
    ...
    # --- MODIFICADO: _export_results_json Correcto y Explícito ---
    def _export_results_json(self):
        ...
        try:
            # Crear un nuevo diccionario para los resultados filtrados
            filtered_summaries = {}

            # Iterar sobre cada estrategia en los datos originales
            for strategy, metrics in self._current_summaries_data.items():
                # Crear un diccionario para las métricas filtradas de esta estrategia
                filtered_metrics = {}
                # Iterar sobre cada métrica de la estrategia actual
                for key, value in metrics.items():
                    # Incluir la métrica SOLO si NO está en la lista de ignorados
                    # y NO es una lista (para excluir manifest/traces implícitamente)
                    if key not in KEYS_TO_IGNORE and not isinstance(value, list):
                        filtered_metrics[key] = value

                # Añadir las métricas filtradas al resultado final, si las hay
                if filtered_metrics:
                    filtered_summaries[strategy] = filtered_metrics

            # Comprobar si quedó algo después de filtrar
            if not filtered_summaries:
                 print("No hay métricas resumidas para exportar después del filtrado.")
                 # Opcional: Mostrar mensaje en UI
                 return

            # Escribir el diccionario filtrado al archivo JSON
            with open(filepath, "w", encoding="utf-8") as f:
                json.dump(filtered_summaries, f, indent=2) # indent=2 para formato legible

            print(f"Resumen de métricas exportado exitosamente a {filepath}")
            # Opcional: Mostrar mensaje de éxito en UI
    ...
    # --- MODIFICADO: Función Principal show_results Refactorizada ---
    def show_results(self, summaries: Optional[Dict[str, Any]]):
        ...
        """
        # 1. Limpiar la vista y guardar los datos completos
        self._clear_results()
        ...
        if "error" in summaries:
            ...
            return
        if not summaries: # Diccionario vacío
            self._show_placeholder()
            return

        # 3. Procesar y Mostrar Métricas Agregadas + Encontrar el primer Manifiesto
        first_manifest = None
        found_valid_strategy = False # Flag para saber si se mostró algo
        for strategy_name, metrics in summaries.items():
            # Saltar claves internas como "_basic", etc.
            if strategy_name.startswith("_"):
                continue

            # Mostrar la tarjeta de métricas si no es una clave interna
            self._add_strategy_card(strategy_name, metrics)
            found_valid_strategy = True # Marcamos que al menos una tarjeta se mostró

            # Intentar obtener el manifiesto de esta estrategia (solo necesitamos uno)
            if first_manifest is None and "files_manifest" in metrics:
                 potential_manifest = metrics.get("files_manifest")
                 # Asegurarse de que sea una lista antes de aceptarlo
                 if isinstance(potential_manifest, list):
                      first_manifest = potential_manifest

        # 4. Poblar la Tabla del Manifiesto
        if isinstance(first_manifest, list): # Verificar de nuevo por seguridad
            ...
            self._populate_manifest_table([]) # Pasar lista vacía si no se encontró manifiesto
            self.manifest_label.configure(text="Manifiesto de Archivos (No disponible)")

        # 5. Habilitar/Deshabilitar Botones de Exportación
        can_export_csv = bool(self._current_manifest_data) # Si hay datos en la tabla
        can_export_json = found_valid_strategy # Si se mostró al menos una tarjeta de métricas
