...
class Disk:
    ...
    def write_block(self, i: int, data: bytes | None) -> None:
        ...
        # Validación de longitud contra block_size
        b = bytes(data)
        if len(b) > self.block_size:
            raise ValueError(
                f"Tamaño de data ({len(b)}) excede block_size ({self.block_size})"
            )
        # Guardamos una copia inmutable
        self._storage[i].set_bytes(b)

    # ---------------------------------------------------------------------
    # Helpers opcionales (no forman parte del contrato, pero son útiles)
    # ---------------------------------------------------------------------
